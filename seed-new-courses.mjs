/**
 * Seed 3 new courses (Tech+, CISM, Network+) into the database.
 * Does NOT delete existing courses (Security+, SecAI+).
 * Run: node seed-new-courses.mjs
 */
...
const seedData = JSON.parse(readFileSync('./new_courses_seed.json', 'utf-8'));
...
async function main() {
  ...
  // Check if new courses already exist
      ...
      for (const lec of mod.lectures) {
        const [lecResult] = await conn.execute(
          ...
          [
            ...
            null,
            null, // audioUrl will be set after S3 upload
